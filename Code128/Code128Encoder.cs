    ...
    public static class Code128Encoder
    {
        public static IBarcodeIntCS Encode(string content, bool includeChecksum = true)
        {
            ...
            if (contentChars.Length <= 0 || contentChars.Length > 80)
                throw new ArgumentException($"Content length should be between 1 and 80 but got {contentChars.Length}",
                    nameof(content));
            ...
            int sum = 0;
            int i = 0;
            foreach (byte idx in idxList.Value.GetBytes())
            {
                if (i == 0)
                    sum = idx;
                else
        ...
        private static BitList? GetCodeIndexList(char[] content)
        {
            ...
            {
                char[] nextChars = content.Skip(i).ToArray();

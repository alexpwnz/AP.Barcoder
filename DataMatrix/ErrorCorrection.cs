...
namespace AP.Barcoder.DataMatrix
{
    ...
    {
        private static readonly ReedSolomonEncoder ReedSolomonEncoder =
            new ReedSolomonEncoder(new GaloisField(301, 256, 1));

        public static byte[] CalculateEcc(byte[] data, CodeSize size)
        {
            int dataSize = data.Length;
            byte[] result = new byte[data.Length + size.EccCount];
            Array.Copy(data, result, data.Length);

            for (int block = 0; block < size.BlockCount; block++)
            {
                int dataCnt = size.DataCodewordsForBlock(block);
                int[] buff = new int[dataCnt];
                ...
                int[] ecc = ReedSolomonEncoder.Encode(buff, size.ErrorCorrectionCodewordsPerBlock);
